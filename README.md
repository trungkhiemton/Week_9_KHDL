...
[cite_start]Dự án này thực hiện đánh giá và so sánh thực nghiệm hiệu năng của thuật toán **FINCH** (First Integer Neighbor Clustering Hierarchy) [cite: 187] với các phương pháp phân cụm phổ biến như **DBSCAN, K-Means, Spectral Clustering, HAC** và **Affinity Propagation (AP)**.
...
- **Giải quyết triệt để rào cản siêu tham số (Hyperparameters):** Trong khi các thuật toán như K-Means, Spectral và HAC bắt buộc người dùng phải cung cấp trước số lượng cụm, hay DBSCAN đòi hỏi tinh chỉnh thủ công các tham số phức tạp (như `eps`), FINCH chứng minh khả năng vận hành hoàn toàn tự động. [cite_start]Thuật toán không cần bất kỳ siêu tham số, ngưỡng khoảng cách hay số lượng cụm định trước nào từ người dùng[cite: 10, 30].

- **Độ ổn định cao & Loại bỏ lỗi "Không hội tụ":** Thực nghiệm cho thấy thuật toán AP thường xuyên gặp tình trạng không hội tụ (`ConvergenceWarning`) khi xử lý dữ liệu. [cite_start]Ngược lại, việc FINCH chỉ sử dụng thông tin láng giềng đầu tiên (First Neighbor) [cite: 9] [cite_start]giúp thuật toán hội tụ rất nhanh qua vài bước và luôn cung cấp một phân vùng hợp lệ tại mỗi bước[cite: 392, 394].
